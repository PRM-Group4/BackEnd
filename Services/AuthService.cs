...
namespace Services
{
        ...
        public class AuthServices : IAuthServices
        {
            ...
            public async Task<BaseResponseForLogin<LoginResponse>> AuthenticateAsync(string username, string password)
            {
                ...
                if (account != null && VerifyPassword(password, account.PasswordHash))
                {
                    ...
                    {
                        return new BaseResponseForLogin<LoginResponse>()
                        {
                            Code = 404,
                            Message = "Your Account has been banned. Check email for reason",
                            Data = new LoginResponse()
                            {
                                Account = new UserResponse()
                                {
